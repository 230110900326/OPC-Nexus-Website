...
describe("ModerationService", () => {
  ...
  const demands = { exists: jest.fn(), findOne: jest.fn(), save: jest.fn() } as unknown as Repository<OpcDemand>;
  const audit = { record: jest.fn() };
  const service = new ModerationService(reports, logs, posts, comments, articles, users, demands, { create: jest.fn() } as never, audit as never); const actor = { id: "33333333-3333-4333-8333-333333333333", email: "mod@example.com", roles: [SystemRole.MODERATOR] };
