...
describe("ModerationService", () => {
  ...
  const service = new ModerationService(reports, logs, posts, comments, articles, users, { create: jest.fn() } as never); const actor = { id: "33333333-3333-4333-8333-333333333333", email: "mod@example.com", roles: [SystemRole.MODERATOR] };
  ...
  it("hides a reported post and writes an audit log", async () => { await service.resolve(report.id, { status: ReportStatus.RESOLVED, action: "hide", resolution: "违反社区规则" }, actor); expect(posts.update).toHaveBeenCalled(); expect(logs.save).toHaveBeenCalled(); expect(report.status).toBe(ReportStatus.RESOLVED); });
});
