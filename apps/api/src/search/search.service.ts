...
export class SearchService {
  ...
  private async searchArticles(input: SearchDto, skip: number, take: number) {
    ...
    query.addSelect("ts_rank(article.search_document, websearch_to_tsquery('simple', :query))", "search_rank").distinct(true).orderBy("search_rank", "DESC").addOrderBy("article.publishedAt", "DESC").skip(skip).take(take);
  ...
  private async searchPosts(input: SearchDto, skip: number, take: number) {
    ...
    query.addSelect("ts_rank(post.search_document, websearch_to_tsquery('simple', :query))", "search_rank").orderBy("search_rank", "DESC").addOrderBy("post.createdAt", "DESC").skip(skip).take(take);
