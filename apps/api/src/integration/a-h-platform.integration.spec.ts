...
describe("A-H platform integration", () => {
  ...
  it("combines classified articles, community posts and authorized videos into one ranked feed and emits a notification", async () => {
    ...
    const feed = await ranking.feed({ mode: FeedMode.RECOMMENDED, scope: RankScope.ALL, window: RankWindow.WEEK }); const feedItems = feed.items; expect(feedItems.map((item) => item.contentType).sort()).toEqual(["article", "post", "video"]); expect(feedItems.every((item) => item.heat >= 0 && item.reason.length > 0)).toBe(true); expect(feedItems.find((item) => item.id === article.id)?.reason).toContain("OPC");
    const notificationRepository = { create: jest.fn((value) => value), save: jest.fn(async (value) => value) } as unknown as Repository<Notification>; await new NotificationsService(notificationRepository).create("dddddddd-dddd-4ddd-8ddd-dddddddddddd", NotificationType.FOLLOWED_AUTHOR_UPDATE, "关注内容更新", feedItems[0].title, feedItems[0].contentType, feedItems[0].id); expect(notificationRepository.save).toHaveBeenCalled();
  });
});
