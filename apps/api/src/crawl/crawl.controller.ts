...
import { RunCrawlNowDto } from "./dto/crawler-run.dto";
...
export class CrawlController {
  constructor(private readonly crawl: CrawlService, private readonly processing: CrawlProcessingService, private readonly runner: CrawlerRunnerService) {}

  @Get() list(@Query() query: ListCrawlSourcesDto) { return this.crawl.list(query).then((data) => ({ success: true, data })); }
  @Get("keywords") keywords() { return { success: true, data: OPC_PRIORITY_KEYWORDS }; }
  @Get("jobs") jobs(@Query("sourceId") sourceId?: string) { return this.crawl.listJobs(sourceId).then((data) => ({ success: true, data })); }
  @Get("jobs/:id/logs") logs(@Param("id", ParseUUIDPipe) id: string) { return this.crawl.listLogs(id).then((data) => ({ success: true, data })); }
  @Post() create(@Body() input: CreateCrawlSourceDto) { return this.crawl.create(input).then((data) => ({ success: true, data })); }
  ...
  @Post("review/:id/link-check") linkCheck(@Param("id", ParseUUIDPipe) id: string, @Body() input: RecordLinkCheckDto) { return this.processing.recordLinkCheck(id, input.statusCode ?? null, input.redirectUrl, input.errorMessage).then((data) => ({ success: true, data })); }
}
