...
export class Article {
  ...
  @Column({ type: "text", default: "" }) content!: string; @Column({ name: "original_content", type: "text", nullable: true }) originalContent!: string | null; @Column({ name: "canonical_url", type: "varchar", length: 1000, nullable: true }) canonicalUrl!: string | null; @Column({ name: "content_fingerprint", type: "varchar", length: 64, nullable: true }) contentFingerprint!: string | null;
  @Column({ name: "classification", type: "jsonb", default: () => "'{}'::jsonb" }) classification!: Record<string, number>; @Column({ name: "summary_keywords", type: "jsonb", default: () => "'[]'::jsonb" }) summaryKeywords!: string[]; @Column({ name: "summary_entities", type: "jsonb", default: () => "'[]'::jsonb" }) summaryEntities!: string[]; @Column({ name: "summary_model_version", type: "varchar", length: 80, nullable: true }) summaryModelVersion!: string | null; @Column({ name: "summary_generated_at", type: "timestamptz", nullable: true }) summaryGeneratedAt!: Date | null; @Column({ name: "summary_reviewed", default: false }) summaryReviewed!: boolean;
  @Column({ name: "agent_analysis", type: "jsonb", default: () => "'{}'::jsonb" }) agentAnalysis!: Record<string, unknown>;
