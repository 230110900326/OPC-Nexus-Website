...
describe("ForumService", () => {
  ...
  const reports = { exists: jest.fn().mockResolvedValue(false), create: jest.fn((value) => value), save: jest.fn() } as unknown as Repository<Report>;
  const service = new ForumService(sections, posts, comments, reports, { create: jest.fn() } as never, { recordDelta: jest.fn() } as never);
