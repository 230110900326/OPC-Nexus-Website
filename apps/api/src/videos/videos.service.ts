...
import { LocalMockVideoAdapter } from "./video-discovery.adapter";
@Injectable()
export class VideosService { constructor(@InjectRepository(Creator) private readonly creators: Repository<Creator>, @InjectRepository(CreatorAccount) private readonly accounts: Repository<CreatorAccount>, @InjectRepository(Video) private readonly videos: Repository<Video>, @InjectRepository(VideoSyncLog) private readonly syncLogs: Repository<VideoSyncLog>) {}
  async list(input: ListVideosDto) {
  ...
  listCreators() { return this.creators.find({ relations: { accounts: true }, order: { trustLevel: "DESC", name: "ASC" } }); }
  async createCreator(input: CreateCreatorDto) { const status = input.authorizationStatus ?? AuthorizationStatus.PENDING; if (input.isEnabled && status !== AuthorizationStatus.AUTHORIZED) throw new BadRequestException("只有已授权创作者可以启用"); return this.creators.save(this.creators.create({ ...input, name: input.name.trim(), avatarUrl: input.avatarUrl ?? null, industries: input.industries ?? [], authorizationStatus: status, isEnabled: input.isEnabled ?? false })); }
  async createAccount(input: CreateCreatorAccountDto) { const creator = await this.creators.findOneBy({ id: input.creatorId }); if (!creator) throw new NotFoundException("创作者不存在"); if (input.isEnabled && (!creator.isEnabled || creator.authorizationStatus !== AuthorizationStatus.AUTHORIZED)) throw new BadRequestException("创作者未获授权或未启用"); return this.accounts.save(this.accounts.create({ ...input, creator, isEnabled: input.isEnabled ?? false })); }
