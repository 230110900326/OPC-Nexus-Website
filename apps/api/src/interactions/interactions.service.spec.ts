...
describe("InteractionsService", () => {
  ...
  const reports = { exists: jest.fn().mockResolvedValue(false), create: jest.fn((value) => value), save: jest.fn() } as unknown as Repository<import("../database/entities/report.entity").Report>;
  const service = new InteractionsService(likes, favorites, follows, articles, posts, comments, users, videos, creators, demands, reports, { inspect: jest.fn().mockResolvedValue({ allowed: true, reason: null }) } as never, { recordDelta: jest.fn() } as never, { create: jest.fn() } as never);
