  ...
  return <main className="demand-editor-page">
    ...
    <section className="demand-editor-shell">
      ...
      <form className="demand-form" onSubmit={save}>
        ...
        <aside className="demand-form-aside"><p className="eyebrow">COMPLIANCE CHECK</p><h2>发布前检查</h2><ul><li className={form.title.length >= 2 ? "done" : ""}>标题具体、可辨识</li><li className={form.content.length >= 20 ? "done" : ""}>交付标准已说明</li><li className={form.industryIds.length > 0 ? "done" : ""}>行业分类已选择</li><li className={form.contactInfo.every((item) => item.value.length >= 3) ? "done" : ""}>联系方式已填写</li></ul><p>{rules || "禁止荐股、代客理财、承诺收益、内幕交易、资金募集或骚扰引流信息。"}</p></aside>
        {error && <p className="form-error" role="alert">{error}</p>}{message && <p className="form-success" role="status">{message}</p>}
        <label className="rules-check"><input type="checkbox" checked={form.agreeToRules} onChange={(event) => setField("agreeToRules", event.target.checked)} />我已阅读并同意 <Link href="/demands/rules" target="_blank">《需求广场服务规范》</Link></label>
