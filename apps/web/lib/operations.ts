...
export type AdminVideo = { id: string; platform: string; title: string; coverUrl: string | null; originalUrl: string; durationSeconds: number; isPublished: boolean; platformMetrics: Record<string, number>; creatorAccount: { displayName: string; creator: { name: string } }; publishedAt: string; industryTags: string[] };
export type AdminVideoPage = { items: AdminVideo[]; pagination: { page: number; limit: number; total: number; totalPages: number } };
export function getAdminVideos(query: Record<string, string | number | undefined> = {}) { const params = new URLSearchParams(); Object.entries(query).forEach(([key, value]) => { if (value !== undefined && value !== "") params.set(key, String(value)); }); return authorizedRequest<AdminVideoPage>(`/admin/videos?${params}`); }
export function toggleVideoPublish(id: string, isPublished: boolean) { return authorizedRequest<AdminVideo>(`/admin/videos/${id}`, { method: "PATCH", body: JSON.stringify({ isPublished }) }); }
